...
type JobDraft struct {
	...
	Outputs        []Output    // The set of expected outputs.  (Some of these may be added, if missing, i.e. the special `Output` that describes stdout and stderr.)
}
...
type Output struct {
	Type       string // `Output.Type` is the repeatr-internal name of what kind of data sink to shovel this output into.  It specifies a handling algorithm.
	Location   string // `Output.Location` is the filepath where this output will be yanked from the job when it reaches completion.
	URL        string // `Output.URL` is a secondary content placement descriptor.  Like Input.URL, it should not be considered to identify the information (a hash is needed for that; an `Output` struct will be generated by the commission of any outputs, and that will contain the suitable `Output.Hash`); but this field may contain extra description like "which ipfs swarm do i coordinate with".

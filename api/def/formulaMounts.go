...
/*
	...
	Use can use `Formula.SetupHash` to get a hash covering the all the pieces
	of the formula used to set up and launch the action: that includes
	all of the type, hash, and mountpath options of every Input.
	The `URI` is *not* included in that hash, because repeatr understands
...
*/
type Input struct {
	Type       string          `json:"type"`           // implementation name (repeatr-internal).  included in Formula.SetupHash.
...
/*
	...
	is not included in the Formula.SetupHash, because repeatr understands that
	your data can be mobile; if you change storage locations, that
	shouldn't bust any caches of known computations).
	...
	One typical example, which is engaged by default, is setting all file
	modification times to a standard value, because this metadata is
	typically both very noisy and very useless.
*/
type Output struct {
	Type       string          `json:"type"`            // implementation name (repeatr-internal).
	Hash       string          `json:"hash"`            // identifying hash of output data.  generated by the output handling implementation during data export when a task is complete.

...
type Executor struct {
	...
	cmdPath       string            // Absolute path to runsc binary.
...
func findGvisorBinary() (string, error) {
	pluginsPath := os.Getenv("REPEATR_PLUGINS_PATH")
	if pluginsPath == "" {
		selfPath, err := os.Executable()
		if err != nil {
			return "", Errorf(repeatr.ErrExecutor, "gvisor executor not available: cannot find plugin: %s", err)
		}
		pluginsPath = filepath.Join(filepath.Dir(selfPath), "plugins")
	}
	expectedPath := filepath.Join(pluginsPath, "repeatr-plugin-runsc")
